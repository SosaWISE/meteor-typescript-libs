...
function convexHull() {
    var width = 960, height = 500;
    var randomX = d3.random.normal(width / 2, 60), randomY = d3.random.normal(height / 2, 60), vertices = d3.range(100).map(function () { return [randomX(), randomY()]; });
    var svg = d3.select("body").append("svg")
        .attr("width", width)
        .attr("height", height)
        .on("mousemove", function () { vertices[0] = d3.mouse(this); redraw(); })
...
function streamGraph() {
    var n = 20, m = 200, stack = d3.layout.stack().offset("wiggle"), layers0 = stack(d3.range(n).map(function () { return bumpLayer(m); })), layers1 = stack(d3.range(n).map(function () { return bumpLayer(m); }));
    var width = 960, height = 500;
    var x = d3.scale.linear()
        .domain([0, m - 1])
        .range([0, width]);
...
function irisParallel() {
    ...
    function brush() {
        var actives = traits.filter(function (p) { return !y[p].brush.empty(); }), extents = actives.map(function (p) { return y[p].brush.extent(); });
...
function extentTest() {
    ...
    d3.extent(["c", "a", "b"], function (d) { return d; });
    d3.extent(["20", "3"], function (d) { return d; });
    d3.extent(["3", "20"], function (d) { return d; });
